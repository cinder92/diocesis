jQuery(function($){
    ...
    $(document).ready(function () {
        ...
        //set megamenu
        var hasElements = !!$('.menu > li:has(ul)')

        if(hasElements){
            //get length and 
            //console.log($('.menu > li:has(ul)'))
            $.each($('.menu > li:has(ul)'),function(i,val){
               var currentLis = $(val).find('li').length
               if(currentLis <= 7){
                 $(val).find('ul').addClass('normal-menu')
               }else{
                 $(val).find('ul').addClass('mega-menu')
               }
                //console.log(i,val)
            })
        }
